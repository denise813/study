...
constexpr void json_item_serialize(const char * item_name, cJSON *root, T & value) {
...
template<typename T, std::enable_if_t<is_int_traits<T>::value, int> = 0>
constexpr void create_json_object(cJSON **root, T & value) {
...
{
    std::shared_ptr<cJSON> j_rootPtr = std::shared_ptr<cJSON>(cJSON_CreateObject(),
                    [](cJSON * root){
                        if (root != nullptr) {
                            cJSON_Delete(root);
                        }
