...
int HTTPDConnHandler::handle_request(TcpConnContextPtr contextPtr)
{
    int rc = 0;
...
int HTTPDConnHandler::handle_response(TcpConnContextPtr contextPtr)
{
    int rc = 0;
    ...
    switch (httpConnContextPtr->getRecode()) {
        case HTTP_RC_CODE_INTERNAL_ERROR: {
            add_status_line(httpConnContextPtr, HTTP_RC_CODE_INTERNAL_ERROR, error_500_title);
            add_headers(httpConnContextPtr, strlen(error_500_form));
            rc = add_content(httpConnContextPtr, error_500_form);
            if (rc != 0) {
                return rc;
            }
            break;
        } case HTTP_RC_CODE_BAD_REQUEST: {
            add_status_line(httpConnContextPtr, HTTP_RC_CODE_BAD_REQUEST, error_404_title);
            add_headers(httpConnContextPtr, strlen(error_404_form));
            rc = add_content(httpConnContextPtr, error_404_form)
            if (rc != 0) {
                return rc;
            }
            break;
        } case HTTP_RC_CODE_FORBIDDEN_REQUEST: {
            add_status_line(httpConnContextPtr, HTTP_RC_CODE_FORBIDDEN_REQUEST, error_403_title);
            add_headers(httpConnContextPtr, strlen(error_403_form));
            rc = add_content(httpConnContextPtr, error_403_form)
            if (rc != 0) {
                return rc;
            }
            break;
        } case HTTP_RC_CODE_OK: {
            add_status_line(httpConnContextPtr, HTTP_RC_CODE_OK, ok_200_title);
            const char *ok_string = httpConnContextPtr->m_resp.getBody().c_str();
            //const char *ok_string = "<html><body></body></html>";
            add_headers(httpConnContextPtr, strlen(ok_string));
            rc = add_content(httpConnContextPtr, ok_string);
            if (rc != 0) {
                return rc;
            }
            break;
